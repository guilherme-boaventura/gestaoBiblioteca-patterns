...
import Classes.MultaDiariaStrategy;
import Classes.MultaFixaStrategy;
import Classes.MultaStrategy;
...
public class Principal {
	...
	public static void main(String[] args) throws ParseException {
		...
		while (opcao != 5) {
			opcao = Integer
					.parseInt(JOptionPane.showInputDialog(null, opcoes, "Biblioteca ", JOptionPane.QUESTION_MESSAGE));
			...
			case 1 -> {
				...
				while (aux != 5) {
					aux = Integer
							.parseInt(JOptionPane.showInputDialog(null, aux1, "Livros ", JOptionPane.QUESTION_MESSAGE));
					...
					case 3 -> {
						livro = biblio
								.obterLivro(JOptionPane.showInputDialog("Digite Título do Livro para pesquisar:"));
						if (livro != null)
							mostrarLivro(livro);
						else
							JOptionPane.showMessageDialog(null, "Livro Não Localizado!");
					...
					case 4 -> {
						...
						for (int i = 0; i < biblio.getQuantidade(); i++) {
							...
							lista.append(livro.getCodigo()).append(" - ").append(livro.getTitulo()).append(" - ")
			...
			case 2 -> {
				...
				while (aux != 5) {
					...
					case 3 -> {
						profe = biblio.obterProfessor(
								JOptionPane.showInputDialog("Digite nome do(a) Professor(a) para pesquisar:"));
						if (profe != null)
							MostrarProfessores(profe);
						else
							JOptionPane.showMessageDialog(null, "Professor(a) Não Localizado!");
					...
					case 4 -> {
						...
						for (int i = 0; i < biblio.getQuantidade1(); i++) {
							...
							lista.append(profe.getCpf()).append(" - ").append(profe.getNome()).append(" - ")
									.append(profe.getDisciplina()).append("; \n");
						}
				...
				while (aux != 5) {
					...
					case 4 -> {
						...
						for (int i = 0; i < biblio.getQuant2(); i++) {
							...
							lista.append(cli.getId()).append(" - ").append(cli.getCpf()).append(" - ")
									.append(cli.getNome()).append(" - ").append(cli.getFone()).append("; \n");
						}
			...
			case 4 -> {
				...
				while (aux != 7) {
					...
					case 2 -> {
						...
						lista.append("ID - ").append(" Leitor - ").append(" Livro - ").append(" Empréstimo - ")
								.append(" Devolução - ").append("; \n");

						for (int i = 0; i < biblio.getQuant3(); i++) {
							empre = biblio.getEmprestimo(i);
							lista.append(empre.getId_emprestimo()).append("   -   ").append(empre.getId_cliente())
									.append("     -     ").append(empre.getId_livro()).append("    -    ")
									.append(empre.getData_emprestimo()).append("      -     ")
	...
	static void mostrarLivro(Livros x) {
		...
		JOptionPane.showMessageDialog(null, texto, "Biblioteca: ", JOptionPane.WARNING_MESSAGE);
	}
	...
	static Emprestimo digitarEmprestimo(Biblioteca biblio) {
			...
			for (int i = 0; i <= biblio.getQuant4(); i++) {
				...
				if (cli != null) {
					if (multa == null || multa.getId_cliente() != cli.getId())
						...
						JOptionPane.showMessageDialog(null, "Leitor(a) possui Multas pendentes!");
				} else
					JOptionPane.showMessageDialog(null, "Leitor(a) Não Localizado!");
			}
		}
			...
			if (liv != null) {
				for (int i = 0; i <= biblio.getQuant4(); i++) {
					...
					if (prof == null || liv.getCodigo() != prof.getControle()) {
						...
						JOptionPane.showMessageDialog(null, "Livro restrito pelo Professor: " + prof.getNome());
				}
			} else
				JOptionPane.showMessageDialog(null, "Livro Não Localizado!");
		}
		...
		// Obtenha o tipo de multa
		String tipoMulta = JOptionPane.showInputDialog("Informe o tipo de multa (Fixa ou Diária):");
		empre.setTipoMulta(tipoMulta);
	...
	static void ListaMulta(Biblioteca biblio) throws ParseException {
		...
		for (int i = 0; i < biblio.getQuant4(); i++) {
			multa = biblio.getMulta(i);
			lista.append(multa.getId_multa()).append(" - ").append(multa.getId_cliente()).append(" - ")
	...
	static Multa DigitarMulta(Biblioteca biblio, Emprestimo empre) {
		...
		multa.setStrategy(strategy);

		float valorMulta = multa.getStrategy().calcularMulta(empre.getData_devolucao());
		...
		if (null != x)
			...
			case "Professor" -> {
				...
				JOptionPane.showMessageDialog(null,
						x + " Cadastrado!\nTotal: " + biblio.getQuantidade1() + " profesor(es)");
			}
			case "Leitor" -> {
				biblio.incluirCliente(digitarCliente(biblio));
				JOptionPane.showMessageDialog(null, x + " Cadastrado!\nTotal: " + biblio.getQuant2() + " Leitor(es)");
			}
			case "Emprestimo" -> {
				biblio.incluirEmprestimo(digitarEmprestimo(biblio));
				JOptionPane.showMessageDialog(null,
						x + " Cadastrado!\nTotal: " + biblio.getQuant3() + " Emprestimos(s)");
			}
			default -> {
			}
			}
